...
import aiohttp
from logger_setup import get_logger  # Используем наш настроенный логгер
from config import API_KEY, API_SECRET, BYBIT_API_URL, BYBIT_WS_PUBLIC_URL, BYBIT_WS_PRIVATE_URL, BYBIT_CATEGORY, \
  USE_TESTNET
...
class BybitConnector:
  def __init__(self, api_key: Optional[str] = API_KEY, api_secret: Optional[str] = API_SECRET):
  ...
  async def close_session(self):
    ...
    if self.exchange:
      ...
      self.exchange = None
      logger.info("CCXT exchange сессия закрыта.")
